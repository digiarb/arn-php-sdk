...
*/
    public function connect($host, $user, $password, $database=false, $persistant=false)
    {
        ...
        {
            throw new Arn_Error('Unable to establish database connection: '
...
*/
    public function importSqlFile($filename, $callback=false, $abort_on_error=true)
    {
        if ($callback && !is_callable($callback)) {
            throw new Arn_Error("Invalid callback function.");
        }
        ...
        foreach ($lines as $num => $line) {
            ...
            if ($end === false) {
                ...
                if (!$result && $abort_on_error) {
                    ...
                    throw new Arn_Error("Error in $file on line $sql_line: $error");
                }
...
*/
    public function smartInsert($table, $columns, $values)
    {
        if (empty($table) || !is_string($table)) {
            throw new Arn_Error('The $table parameter must be specified as a string.');
        }
        
        $table_sql = '`' . @mysqli_real_escape_string($this->link, $table) . '`';
        ...
        if (is_array($columns)) {
            foreach ($columns as &$col) {
                if (!is_string($col)) {
                    throw new Arn_Error('The $columns parameter must be a string or an array of strings');
                }
                $col = @mysqli_real_escape_string($this->link, $col);
            }
        ...
        try {
            ...
            throw new Arn_Error("Could not get column information for table $table_sql.");
        }
        ...
        if (is_array($values)) {
            ...
            throw new Arn_Error('The $values parameter must be a string or an array.');
        }
        
        if (isset($column_count) && $column_count <> $value_count) {
            throw new Arn_Error("Column count ($column_count) does not match values count ($value_count).");
        }
...
*/
    private function loadFile($filename)
    {
        if (!file_exists($filename)) {
            throw new Arn_Error("File does not exist: $filename");
        }
        
        $file = @file($filename, FILE_IGNORE_NEW_LINES);
        
        if (!$file) {
            throw new Arn_Error("Could not open $filename");
        }
    ...
    public function query($query)
    {
        ...
        if (!$r) {
            throw new Arn_Error("Query Error: " . mysqli_error($this->link));
        }
...
*/
    public function useDatabase($database)
    {
        ...
        {
            throw new Arn_Error('Unable to select database: ' . mysqli_error($this->link));
        }
    }
}
